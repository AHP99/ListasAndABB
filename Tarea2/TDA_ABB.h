 ...
 *******************
 * Resumen Función: 
   Inserta un elemento en la posicion correcta segun la jerarquia de un ABB, ayudandose de insertHe
 ********************
 * input:
   t* T: Puntero al arbol en el cual se quiere insertar un nodo.reeNode **N: Puntero a un puntero de tipo treeNode, es el nodo que se esta revisando en esa ejecucion de la funcion
 ...
 *******************
 * Resumen Función: 
   Recorre el árbol desde su raíz hacia la derecha, en búsqueda del nodo que contenga el elemento más grande del árbol.
 ********************
 * input:
   tABB*T: Puntero a el arbol en el que se buscara el máximo.
 ********************
 * Returns:
   tipoElem: el valor del nodo máximo que contiene el ábrol.
 ********************/
 ...
 *******************
 * Resumen Función: 
   Separa los casos de búsqueda del sucesor dentro del arbol de un elemento dado. Estos casos pueden ser: Cuando el elemento
   dado está en el arbol se divide en casos en que tiene hijo derecho (se buscará al sucesor en su sub árbol derecho), en que 
   el nodo no tenga hijo derecho (se buscará al sucesor sobre el nodo), cuando es el nodo máximo en el arbol o excede a este (se retornará u).
   Casos en que el elemento no es un nodo del árbol, en cuyo caso se siguen otros algoritmos de bpusqueda, ayudános con sucesorHelp().
 ********************
 * input:
   tABB *T: Puntero a el arbol en el que se buscara el sucesor de un elemento dado.
   tipoElemen item: Elemento del cual se quiere buscar un sucesor en el árbol.
   int max: La cota superior de los nodos del árbol, es decir el número u leído del archivo.
 ********************
 * Returns:
   treeNode*, puntero al nodo que contiene al sucesor del elemento (item) dado.
 ********************/
 ...
 *******************
 * Resumen Función: 
   Borra un nodo, en el arbol T, separando por casos de borrado; suando el nodo es hoja, cuando tiene un sólo hijo
   a su izquierda, a su derecha, y cuando tiene ambos hijos, reemplacándo el nodo borrado por su predecesor.
 ********************
 * input:
   tABB *T: Puntero a el arbol en el que se borrará un elemento
   treeNode *ParaBorrar: Puntero al nodo del arbol que queremos borrar. 
 ********************
 * Returns:
   Nada; función de tipo void.
 ********************/
